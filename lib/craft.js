...
function makeAdvancedFilters(params) {
    const { tbs, safe, nfpr, filter } = params || {};
    /*
...
function makeScreen(params) {
    const { biw, bih } = params || {};
...
function craft(options) {
    const {
        protocol = constants.HTTP_PROTOCOL, override, exclude, shuffle = true,
        ...googleOptions
    } = options || {};

    const { hostname, uule, cr, gl, hl, lr } = parseLocation(options);

    const params = { ...googleOptions, uule, cr, gl, hl, lr };

    const url = new URL(`${protocol}://${hostname}${constants.GOOGLE_SEARCH_PATH}`);

    const search = utils.organizeObject({
